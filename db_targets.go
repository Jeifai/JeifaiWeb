...
func (user *User) DeleteUserTargetsKeywordsByKeyword(keyword string) {
...
func (user *User) InsertUserTargetsKeywords(keywords []string, targets []string) {
	...
	k_rows, err := Db.Query(`
					...
					LEFT JOIN keywords k ON(uk.keywordid = k.id)
					WHERE uk.userid = $1
					AND k.text IN($2);`, user.Id, pq.Array(keywords))
	if err != nil {
		panic(err.Error())
	}
	for k_rows.Next() {
	...
	t_rows, err := Db.Query(`
					...
					LEFT JOIN targets t ON(ut.targetid = t.id)
					WHERE ut.userid = $1
					AND t.name IN($2);`, user.Id, pq.Array(targets))
	if err != nil {
		panic(err.Error())
	}
	for t_rows.Next() {
