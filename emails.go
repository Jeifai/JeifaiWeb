...
func (target *Target) SendEmailToAdminAboutNewTarget() {
	fmt.Println(Gray(8-1, "Starting SendEmailToAdminAboutNewTarget..."))
	err := godotenv.Load()
	if err != nil {
		panic(err.Error())
	}
	...
	t := template.New("SendEmailToAdminAboutNewTarget.html")

	t, err = t.ParseFiles("templates/emails/SendEmailToAdminAboutNewTarget.html")
	if err != nil {
		panic(err.Error())
	}
	...
	var tpl bytes.Buffer
	if err := t.Execute(&tpl, target); err != nil {
		fmt.Println(err)
	}
	...
	m.SetHeader("From", "robimalco@gmail.com")
	m.SetHeader("To", "robimalco@gmail.com")
	m.SetHeader("Subject", "NEW TARGET REQUEST RECEIVED")
	m.SetBody("text/html", result)
	...
	SaveEmailIntoDb("robimalco@gmail.com", "SendEmailToAdminAboutNewTarget")
}
